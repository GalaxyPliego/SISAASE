...
public class DaoHistorial {
    ...
    public List<BeanPeriodoCuatrimestral> findPeriodoCuatrimestral() throws SQLException {
        ...
        try{
            ...
            String script = "SELECT * FROM sisa_ase.periodo_cuatrimestral order by periodoAxo DESC, nombreCuatrimestre DESC;";
    ...
    public List<BeanAsesorias> findHistorialGeneral() throws SQLException {
        ...
        List<BeanPeriodoCuatrimestral> periodos = new DaoHistorial().findPeriodoCuatrimestral();
        BeanPeriodoCuatrimestral periodoActual = periodos.get(0);
        try{
            connection = ConnectionMysql.getConnection();
            ...
            pstm = connection.prepareStatement(script);
            pstm.setInt(1, periodoActual.getIdPeriodoCuatrimestral());
            pstm.setInt(2, 1);
            rs = pstm.executeQuery();
            while (rs.next()){
    ...
    public List<BeanAsesorias> cambioPeriodoHistorialGeneral(int periodoCuatrimestral, int idCarrera) throws SQLException {
        List<BeanAsesorias> listHistorialGeneral = new ArrayList<>();
        ...
        try{
            ...
            String script = "SELECT ai.matricula, ai.Nombres, ai.aPaterno, ai.Sexo, ca.nombrecarrera, g.cuatrimestre, \n" +
                    ...
                    "WHERE ase.idPeriodoCuatrimestral = ? AND ca.idCarrera = ?;";
            pstm = connection.prepareStatement(script);
            pstm.setInt(1, periodoCuatrimestral);
            pstm.setInt(2, idCarrera);
            rs = pstm.executeQuery();
            while (rs.next()){
                BeanAsesorias asesoria = new BeanAsesorias();
                BeanAlumnoInscrito alumnoInscrito = new BeanAlumnoInscrito();
...
//                ));
    ...
    public List<BeanAsesorias> findHistorialDocente() throws SQLException {
        ...
        try{
            ...
            pstm = connection.prepareStatement(script);
            pstm.setInt(1, periodoActual.getIdPeriodoCuatrimestral());
            pstm.setInt(2, Integer.parseInt(beanUsuario.getClaveIdentidad()));
            rs = pstm.executeQuery();
            while (rs.next()){
                BeanAlumnoInscrito alumnoInscrito = new BeanAlumnoInscrito();
                BeanAsesorias asesoria = new BeanAsesorias();
